...
public class InputMove : Move
{
	...
	public override (bool is_actionable, SimulatedData result) Simulate()
	{
		...
		if(!IsMovable(_next_mass))	// 移動不可能
		{
			...
			{
				// 更新
				_next_mass = _DungeonScene.FloorData.MapData.Masses[_currect_mass_idx.y, _next_mass_idx.x];	// x成分に沿った移動で再度試す

				// 検査
				if(!IsMovable(_next_mass))	// x方向にも移動できない
				{
					// 更新
					_next_mass = _DungeonScene.FloorData.MapData.Masses[_next_mass_idx.y, _currect_mass_idx.x];	// y成分に沿った移動で再度試す
